  ...
  const highlights = Array.isArray(llm.risk_highlights) && llm.risk_highlights.length
    ...
    : computed
      ? [
          ...
          `주요 부위: ${computed.main_body_parts?.join(", ") || "알 수 없음"}`,
        ]
      : [];
...
function buildFirstSummary(llm, computed) {
  ...
  return {
    headline: source.headline || fallbackText,
    risk_level_summary: source.risk_level_summary || fallbackText,
    main_risk_cause: source.main_risk_cause || "주요 위험 원인을 계산된 근거와 함께 확인해야 합니다.",
    priority_action: source.priority_action || fallbackActions[0] || "작업 대상의 위치와 높이를 먼저 조정하세요.",
    focus_time_range: source.focus_time_range || "대표 고위험 구간을 먼저 확인하세요.",
    top_3_actions: Array.isArray(source.top_3_actions) && source.top_3_actions.length
      ? source.top_3_actions.slice(0, 3)
